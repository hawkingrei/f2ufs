...
use crate::util::lru::{CountMeter, Lru, Pinnable};
    ...
    T: Cowable + Deserialize<'de> + Serialize + 'static,
{
    ...
    fn add_to_trans(&mut self, action: Action) -> Result<()> {
        ...
        // add cow to transaction
        {
            ...
            txmgr.add_to_trans(&self.id, curr_txid, self_ref, action, EntityType::Cow, arm)?;
        }
    ...
    pub fn load(id: &Eid, txmgr: &TxMgrRef, vol: &VolumeRef) -> Result<CowRef<T>> {
    ...
    T: Cowable + Deserialize<'de> + Serialize + 'static,
{
    ...
    fn deref(&self) -> &T {
        ...
        if self.txid.is_none() || self.txid != Some(curr_txid) || self.action == Some(Action::New) {
            self.inner()
        } else {
            self.other_inner()
        }
...
{
    fn into_cow_with_id(self, id: &Eid, txmgr: &TxMgrRef) -> Result<CowRef<Self>> {
