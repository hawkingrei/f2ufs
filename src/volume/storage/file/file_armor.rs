...
use crate::volume::armor::{ArmAccess, Armor};
...
impl Read for CryptoReader {
    ...
    fn read(&mut self, buf: &mut [u8]) -> IoResult<usize> {
        ...
        buf[..copy_len].copy_from_slice(&self.frame[self.read..self.read + copy_len]);
        self.read += copy_len;
...
impl CryptoWriter {
    ...
    fn write_frame(&mut self) -> Result<()> {
        ...
        self.frame_len =
            self.crypto
                .encrypt_to(&mut self.frame, &self.stg[..self.stg_len], &self.key)?;
...
impl<'de, T: ArmAccess<'de> + Debug> Armor<'de> for FileArmor<T> {
    ...
    fn get_item_reader(&self, arm_id: &Eid) -> Result<Self::ItemReader> {
        ...
        match vio::OpenOptions::new().read(true).open(&path) {
            ...
            Err(ref err) if err.kind() == ErrorKind::NotFound => Err(Error::NotFound),
