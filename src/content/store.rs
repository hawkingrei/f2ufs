...
use super::content::{Cache as ContentCache, ContentRef, Writer as ContentWriter};
use super::segment::{
    Cache as SegCache, DataCache as SegDataCache, SegData, SegDataRef, SegRef, Segment,
};
use super::Content;
use crate::error::{Error, Result};
use crate::trans::cow::{Cow, CowRef, Cowable, IntoCow};
use crate::trans::trans::Action;
...
impl Store {
    ...
    pub fn open(store_id: &Eid, txmgr: &TxMgrRef, vol: &VolumeRef) -> Result<StoreRef> {
        let store = Cow::<Store>::load(store_id, txmgr, vol)?;
        {
            let mut store_cow = store.write().unwrap();
            let store = store_cow.make_mut_naive();
            store.content_cache = ContentCache::new(Self::CONTENT_CACHE_SIZE, txmgr);
    ...
    pub fn deref_content(&mut self, content_id: &Eid) -> Result<Option<ContentRef>> {
        {
    ...
    /// Shrink segment
    pub fn shrink_segment(&mut self, seg_cow: &mut Cow<Segment>) -> Result<Vec<usize>> {
        ...
        SegData::add_to_trans(seg.data_id(), Action::Delete, txid, &self.txmgr)?;
...
impl Writer {
    pub fn new(chk_map: ChunkMap, txmgr: &TxMgrRef, store: &StoreRef, txid: Txid) -> Self {
