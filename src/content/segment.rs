...
use crate::trans::{Eid, EntityType, Finish, Id, TxMgrRef, Txid};
...
impl SegData {
    ...
    pub fn add_to_trans(data_id: &Eid, action: Action, txid: Txid, txmgr: &TxMgrRef) -> Result<()> {
    ...
    pub fn shrink(
        ...
        for (idx, chunk) in self.chunks.iter_mut().enumerate() {
            if chunk.is_orphan() {
                ...
                buf.extend_from_slice(&seg_data.data[chunk.pos..chunk.end_pos()]);
...
impl Writer {
    pub fn new(txid: Txid, store: &StoreRef, txmgr: &TxMgrRef, vol: &VolumeRef) -> Self {
        Writer {
            seg: Arc::default(),
            data_wtr: None,
            txid,
    ...
    pub fn renew(&mut self) -> Result<()> {
        ...
        // segment data itself
        SegData::add_to_trans(&seg.data_id, Action::New, self.txid, &self.txmgr)?;
